...
internal sealed class ConstructorNoContext<P1> : DyForeignFunction
    where P1 : DyObject
{
    private readonly Func<P1, DyObject> fun;

    public ConstructorNoContext(string name, Func< P1, DyObject> fun, Par par)
        : base(name, new[] { par }) => (this.fun, Attr) = (fun, FunAttr.Vari);

    internal override DyObject InternalCall(ExecutionContext ctx, DyObject[] args) => fun(Cast<P1>(0, args)!);
