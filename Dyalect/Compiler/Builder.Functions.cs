    ...
    partial class Builder
    {
        private void Build(DFunctionDeclaration node, Hints hints, CompilerContext ctx)
        {
            if (node.Name != null)
            {
                ...
                else if (node.Attribute == FunctionAttribute.Constructor)
        ...
        private void BuildFunctionBody(int addr, DFunctionDeclaration node, Hints hints)
        {
            ...
            if (node.IsMemberFunction && !node.IsStatic)
            {
                ...
                if (node.Attribute == FunctionAttribute.Deconstructor)
                    cw.Unbox();
            ...
            if (node.Attribute == FunctionAttribute.Constructor && node.Body is null)
            ...
            //Declared with "ctor" keyword
            if (node.Attribute == FunctionAttribute.Constructor && node.Body is not null)
            {
                if (node.IsIterator)
                    AddError(CompilerError.CtorNotIterator, node.Location);

                if (node.TypeName.Parent != null || !TryGetLocalType(node.TypeName.Local, out var ti))
