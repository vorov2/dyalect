...
namespace Dyalect.Library.Core
{
    public sealed class DyRegexTypeInfo : DyForeignTypeInfo
    {
        ...
        private DyObject StaticSplit(ExecutionContext ctx, DyObject input, DyObject pattern, DyObject ignoreCase,
            DyObject removeEmptyEntries)
        {
            if (input.TypeId != DyType.String)
                return ctx.InvalidType(DyType.String, input);
            ...
            var arr = Regex.Split(input.GetString(), pattern.GetString(),
                ignoreCase.GetBool(ctx) ? RegexOptions.IgnoreCase : RegexOptions.None);
            var objs = new List<DyObject>();
            var flag = removeEmptyEntries.GetBool(ctx);
        ...
        protected override DyFunction? InitializeInstanceMember(DyObject self, string name, ExecutionContext ctx) =>
            name switch
            {
                ...
                "Replace" => Func.Member(name, Replace, -1, new Par("value"), new Par("replacement")),
                "Split" => Func.Member(name, Split, -1, new Par("value"), new Par("count", DyInteger.Zero),
        ...
        protected override DyFunction? InitializeStaticMember(string name, ExecutionContext ctx) =>
            name switch
            {
                ...
                "Replace" => Func.Static(name, StaticReplace, -1, new Par("value"), new Par("pattern"), new Par("replacement"), new Par("ignoreCase", DyBool.False)),
                "Split" => Func.Static(name, StaticSplit, -1, new Par("value"), new Par("pattern"),
