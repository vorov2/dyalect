...
public class _DyDateTime : DyForeignObject, IDateTime, IFormattable
{
    private const string FORMAT = "yyyy-MM-dd HH:mm:ss.fffffff";
    
    private long ticks;

    internal _DyDateTime(AbstractDateTimeTypeInfo<_DyDateTime> typeInfo, long ticks) : base(typeInfo) =>
        this.ticks = ticks;

    public override object ToObject() => new DateTime(ticks);

    public override DyObject Clone() => new _DyDateTime((AbstractDateTimeTypeInfo<_DyDateTime>)TypeInfo, ticks);

    public override bool Equals(DyObject? other) => other is _DyDateTime dt && dt.ticks == ticks;

    public override int GetHashCode() => ticks.GetHashCode();

    public override string ToString() => ToString(FORMAT);

    public static _DyDateTime Parse(DyForeignTypeInfo typeInfo, string format, string value)
    {
        var (ticks, _) = InputParser.Parse(FormatParser.DateTimeParser, format, value);
        return new((AbstractDateTimeTypeInfo<_DyDateTime>)typeInfo, ticks);
    }
