    ...
    internal static class StringUtil
    {
        ...
        public static string ToLiteral(this object obj)
        {
            if (obj is string str)
                return $"new {SourceGenerator.Types.DyString}({Escape(str)})";
            else if (obj is bool b)
                return b ? $"{SourceGenerator.Types.DyBool}.True" : $"{SourceGenerator.Types.DyBool}.False";
            else if (obj is char c)
                return $"new {SourceGenerator.Types.DyChar}({Escape(c.ToString(), "'")})";
